...
const formattedDate = isoDateString.slice(0, 17) + '00Z';
...
app.patch('/posts/:id', (req, res) => {
  const id = parseInt(req.params.id);
  const existingBlog = posts.find((post) => post.id === id);
  const updatedBlog = {
    id: id,
  ...
  const blogIndex = posts.findIndex((post) => post.id === id);
  posts[blogIndex] = updatedBlog;
